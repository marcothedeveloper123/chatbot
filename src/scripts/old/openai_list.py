...
def _run():  # we only call this from the command line
    try:
        while True:
            ...
            if user_prompt.startswith(
                ...
                chatbot.system_prompt = user_prompt[3:].strip()
                ...
                chatbot.add_user_prompt(user_prompt)
                """
                the user prompt does not start with `-s`, so we treat it as a prompt to the LLM
                """
                if chatbot.streaming:  # we run streaming reponses through a `for` loop
...
chatbot = Chatbot(  # generate the Chatbot() object that mediates between user and LLM
