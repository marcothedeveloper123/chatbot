...
class Chat {
	...
	bindEvents() {
		...
		this.userInput.addEventListener('input', function() {
			// Reset height to ensure we are calculating from the base height
			this.style.height = 'auto';

			// Set the height to scroll height to remove the scrollbar
			this.style.height = this.scrollHeight + 'px';

			// Limit the height to one-third of the window height
		...
		this.socket.on('end_chatbot_response', () => {
			...
			if (chatbotResponseElement) {
				...
				const decodedContent = this.decodeHtmlEntities(parsedContent);
				...
				messageContentElement.innerHTML = decodedContent;
 	...
 	* @return {string} A new string with HTML entities decoded.
 	*/
	...
	adjustTextareaHeight() {
		this.userInput.style.height = 'auto'; // Reset height to shrink back if text is removed
